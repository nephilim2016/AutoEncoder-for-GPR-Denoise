...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 27 01:12:32 2020
...
if __name__=='__main__':
    ...
    for iteration in range(Iteration):
        if iteration in [6,7,18,33]:
            continue
        Profile=np.load('./GPR_Modelling/Profile_TunnelLining_Stochastic900MB/TunnelLining_Iter_%s_compare_%s.npy'%(iteration,compare))
        ProfileGain=T_PowerGain.tpowGain(Profile,np.arange(5000)/4,0.9)
        ...
        # print(np.max(ProfileGain_Patch))
    ...
    for iteration in range(Iteration):
        if iteration in [6,7,18,33]:
            continue
        Profile=np.load('./GPR_Modelling/Profile_TunnelLining_Stochastic900MB/TunnelLining_Iter_%s_compare_%s.npy'%(iteration,compare))
        ProfileNoiseGain=T_PowerGain.tpowGain(Profile,np.arange(5000)/4,0.9)
        ...
        print(np.max(ProfileNoiseGain_Patch))
    ...
    # AutoEncoder=Build_AutoEncoder.BuildDropOutAutoEncoder(ImageShape=tuple(list(patch_size)+[1,]),filters=[16,32,64],kernel_size=[5,5,5],latent_dim=256)
    ...
    epochs=10
    save_path_name='./StochasticTunnelLiningNormalAutoEncoder400MB900MB_iteration10_monitor_Resdualkernel5_windows3_[16,32,64]'
    ...
    # Read Target Profile
    ProfileTarget=np.load('./GPR_Modelling/Profile_TunnelLining_Stochastic900MB/TunnelLining_Iter_%s_compare_%s.npy'%(10,1))
    ProfileTargetGain=T_PowerGain.tpowGain(ProfileTarget,np.arange(5000)/4,0.9)
    ...
    ProfileClean=np.load('./GPR_Modelling/Profile_TunnelLining_Stochastic900MB/TunnelLining_Iter_%s_compare_%s.npy'%(10,0))
    ProfileClean=T_PowerGain.tpowGain(ProfileClean,np.arange(5000)/4,0.9)
    ...
    DisplayFigure.PlotResultFigure(ProfileClean,ProfileTargetGain,Mode,patch_size,slidingDis,True)
